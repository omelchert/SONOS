...
def main():
        ...
        p_rD_t   = OASignal((zD,rhoD),(z,dz,S_z),(rho,drho,phi,dphi,iProf),dr)

        print "# (t) \t (c tau) \t (p(rD,t))"
        for i in range(p_rD_t.size):
            print "%g \t %g \t %g"%(i*dz/c0,i*dz+zD, p_rD_t[i])
